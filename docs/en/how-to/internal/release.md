...
2. Make sure the branch is ready for release. Ensure that:
    ...
    2. The release notes are up to date. If they are, the [changes](https://github.com/beeware/xbuild/tree/main/changes) directory should be empty, except for the `template.rst` file.
...
4. Pushing the tag will start a workflow to create a draft release on GitHub. You can [follow the progress of the workflow on GitHub](https://github.com/beeware/xbuild/actions?query=workflow%3A%22Create+Release%22); once the workflow completes, there should be a new [draft release](https://github.com/beeware/xbuild/releases).
...
6. Double check everything, then click Publish. This will trigger a [publication workflow on GitHub](https://github.com/beeware/xbuild/actions?query=workflow%3A%22Upload+Python+Package%22).

7. Wait for the [package to appear on PyPI](https://pypi.org/project/xbuild/).
